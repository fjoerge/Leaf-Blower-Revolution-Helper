...
$config = [pscustomobject]@{
    ...
    # Trade All Other: aktiviert das Traden aller Items ausser Beer, Gems, Cheese, Mulch, Borbs
    TradeAllOther = $true
    ItemPolicies = @{
        Gem = @{
            Start         = $false
        ...
        Any = @{
        Start         = $false  # Wird über TradeAllOther gesteuert
        Tolerance     = 100      # Höhere Toleranz = passt auf mehr Items
        NeedsGemValue = $false
        }
...
# --- Pruefen ob Item zu den konfigurierbaren Items gehoert --------------------

<#
    Prueft, ob ein Item-Name zu den bekannten, ueber GUI konfigurierbaren Items gehoert.
    Diese Items sind: Beer, Gem, Cheese, Mulch, Borb
    
    Returnwert: $true wenn es ein bekanntes Item ist, sonst $false
#>
function Test-IsKnownConfigurableItem {
    param(
        [string]$ItemName
    )
    
    $knownItems = @("Beer", "Gem", "Cheese", "Mulch", "Borb")
    return ($knownItems -contains $ItemName)
}

# --- Zentrale Trade-Entscheidung --------------------------------------------

<#
    Entscheidet zentral, ob ein Item-Trade gestartet werden soll.
    Beruecksichtigt ItemPolicies und die TradeAllOther Option.
    
    Logik:
    - Bekannte Items (Beer, Gem, Cheese, Mulch, Borb): nur wenn .Start = $true
    - Unbekannte Items: nur wenn TradeAllOther = $true
    - Bei Gems: zusaetzlich MinValue-Check
#>
function Should-StartItemTrade {
    ...
    $isKnownItem = Test-IsKnownConfigurableItem -ItemName $ItemName
    
    if ($isKnownItem) {
        # Bekannte Items: normale Policy-Logik
        $policy = $config.ItemPolicies[$ItemName]
        
        if (-not $policy) {
            Write-Log "Item '$ItemName' hat keine Policy-Definition -> wird nicht getradet." "DEBUG"
            return $false
        }
        
        # Item muss explizit aktiviert sein
        if (-not $policy.Start) { 
            return $false 
        }
        
        # Gem-spezifisch: MinValue pruefen
        if ($policy.NeedsGemValue) {
            if ($policy.ContainsKey("MinValue")) {
                # Nur Pruefen ob Gestartet werden soll, Wert ggf. noch unbekannt
                if ($PreGemValue -eq 0) {
                    Write-Log ("Item = {0}: Value=0, ShouldTrade = {1} -> noch kein OCR & check MinValue={2} ueberspringen" -f $ItemName, $policy.Start, $policy.MinValue) "DEBUG"
                    return $true
                }

                # hier auf "MinValue" pruefen
                if ($PreGemValue -lt $policy.MinValue) {
                    Write-Log ("Item = {0}: GemValue = {1} < MinValue = {2} -> wird nicht getradet" -f $ItemName, $PreGemValue, $policy.MinValue) "DEBUG"
                    return $false
                }
            }
        }
        ...
        # Unbekannte Items: nur traden wenn TradeAllOther aktiviert ist
        if ($config.TradeAllOther) {
            Write-Log ("Item '{0}' ist unbekannt, aber TradeAllOther=true -> wird getradet" -f $ItemName) "DEBUG"
            return $true
        }
        else {
            Write-Log ("Item '{0}' ist unbekannt und TradeAllOther=false -> wird NICHT getradet" -f $ItemName) "DEBUG"
            return $false
        }
    }
}
...
$itemDefs = @(
    ...
    @{ Name = "Any"; File = "AnySymbol.png"; Tolerance = $config.ItemPolicies.Any.Tolerance }
)
