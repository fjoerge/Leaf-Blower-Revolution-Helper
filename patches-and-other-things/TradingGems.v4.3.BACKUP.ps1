<#
Leaf Blower Revolution - Trade Automation Version 4.2
#>
...
function Get-GemValue {
    ...
    if ($brightPixelCount -lt 2) {
        ...
        write-host "Line 1158" -ForegroundColor Red
...
#>function Should-StartItemTrade {
    ...
    if ($policy.NeedsGemValue) {
        if ($PreGemValue -eq $null -or $PreGemValue -le 0) { return $false }

        # hier auf "MinValue" pruefen, nicht auf den Zahlenwert aus der Config
        if ($policy.ContainsKey("MinValue")) {
            if ($PreGemValue -lt $policy.MinValue) { return $false }
        }
    }
...
function Try-StartTrade {
    ...
    if ($isDebugMode) {
        if ($PreGemValue -ne $null -and $PreGemValue -gt 0) {
            ...
            Write-Log ("Y={0}: GemValue vor Startversuchen = none/null" -f $ClickY) "DEBUG"
        }
    }
    ...
    for ($startAttempt = 1; $startAttempt -le $maxStartAttempts; $startAttempt++) {
        ...
        if ($slotRunningAfterClick) {
            ...
            if ($PreGemValue -ne $null -and $PreGemValue -gt 0) {
                ...
                Write-Log ("Y={0}: Kein gueltiger Gem-Wert ermittelt, Trade aber gestartet." -f $ClickY) "INFO"
            }
...
function Invoke-CollectRefresh {
    ...
    Start-Sleep -Milliseconds $config.PostCollectDelayMs
}

<#
    Fuehrt nur das Refresh-Makro aus (kein Collect) und erhoeht RefreshCount.
    Wird genutzt, wenn Slots nicht voll sind oder Collect noch nicht faellig ist.
#>
...
$itemDefs = @(
    ...
    @{ Name = "CosmicLeaf"; File = "CosmicLeafSymbol.png"; Tolerance = $config.ItemPolicies.CosmicLeaf.Tolerance }
)
...
while ($true) {
    ...
    foreach ($hit in $symbolHits) {
        ...
        # Item-Typ ermitteln (Fallback = "Gem", falls kein Item-Feld vorhanden)
        ...
        # 3) Start-Button auf "disabled" pruefen (gemeinsam fuer alle Item-Typen)
        ...
        # 4) Item-spezifische Behandlung mit Policy
        if ($itemName -eq "Gem") {
            ...
            if (-not (Should-StartItemTrade -ItemName $itemName -PreGemValue $null)) {
